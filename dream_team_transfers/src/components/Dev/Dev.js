...
function Dev() {
    ...
    const ALL_PICTURES = Object.entries(PREMIER_LEAGUE_TEAM_INFOS)
    .flatMap(([key, value]) =>
    ...
    useEffect(() => {
        ...
        const teamPhotos = PREMIER_LEAGUE_TEAM_INFOS[currTeam].team_photos.map((url, index) => (
            <div className='team-photo' key={`${currTeam}-${index}`}>
                     <h2>{PREMIER_LEAGUE_TEAM_INFOS[currTeam].alias}-{index}</h2>
    ...
    return (
        <div className="dev-body">
            ...
            <button onClick={() => setCurrTeam(currTeam - 1)}>
                Prev Team
            </button>
            <button onClick={() => setCurrTeam(currTeam + 1)}
                >Next Team
